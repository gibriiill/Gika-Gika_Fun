...
/**
 * Vanilla-JS port of the React Bits "LiquidEther" background
 * (https://reactbits.dev/backgrounds/liquid-ether), adapted to run
 * without React so it can be dropped into a plain HTML/CSS/JS page.
 *
 * Usage:
 *   import { createLiquidEther } from './liquid-ether.js';
 *   const fx = createLiquidEther(document.querySelector('#bg'), {
 *     colors: ['#2c8478', '#175c56', '#eef4ef']
 *   });
 *   // later, if needed: fx.destroy();
 */
export function createLiquidEther(container, {
  mouseForce = 20,
  cursorSize = 100,
  isViscous = false,
  viscous = 30,
  ...
  let webglRef = { current: null };
  let resizeObserverRef = { current: null };
  let rafRef = { current: null };
  let intersectionObserverRef = { current: null };
  let isVisibleRef = { current: true };
  let resizeRafRef = { current: null };
